...
import {
  agregarGastoCompartido,
  actualizarEstadoGastoCompartido,
  actualizarGastoCompartido,
  subirComprobanteCompartido,
  eliminarGastoCompartido,
} from '@/app/compartido/actions';
import { CATEGORIAS_GASTOS_COMPARTIDOS } from '@/lib/compartido/types';
import type { GastoCompartido } from '@/lib/compartido/types';
import type { useToasts } from '@/lib/useToasts';
import { ConfirmButton } from '@/components/finanzas/ConfirmButton';

interface GastosCompartidosPanelProps {
  gastos: GastoCompartido[];
  anioMes: string;
  toasts: ReturnType<typeof useToasts>;
}

export function GastosCompartidosPanel({ gastos, anioMes, toasts }: GastosCompartidosPanelProps) {
  ...
  const handleAgregar = (formData: FormData) => {
    ...
    startTransition(async () => {
      try {
        await agregarGastoCompartido(formData);
        toasts.success('Gasto compartido registrado.');
      } catch (err) {
  ...
  const handleToggleEstado = (g: GastoCompartido) => {
    startTransition(async () => {
      try {
        await actualizarEstadoGastoCompartido(g.id, g.estado === 'Pagado' ? 'Pendiente' : 'Pagado');
  ...
  const handleEditarCampo = (
    ...
    startTransition(async () => {
      try {
        await actualizarGastoCompartido(id, campo, valor);
  ...
  const handleUpload = async (id: string, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      await subirComprobanteCompartido(id, formData);
  ...
  const handleEliminar = (id: string) => {
    startTransition(async () => {
      try {
        await eliminarGastoCompartido(id);
  ...
  return (
    <section className="space-y-4">
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        <h2 className="font-fenix text-xl text-zinc-300 font-normal">Gastos Compartidos</h2>
        <div className="text-right text-xs font-mono text-zinc-500">
          ...
          {pendiente > 0 && (
            <span className="text-amber-400 ml-2">Pendiente: ${pendiente.toLocaleString('es-CO')}</span>
          )}
        </div>
      </div>
      ...
      <div className="md:hidden space-y-3">
        {gastos.length === 0 && (
          <p className="py-8 text-center text-zinc-600 text-xs font-mono rounded-2xl border border-zinc-900">
            No hay gastos compartidos registrados para este periodo.
          </p>
        )}
        {gastos.map((g) => (
          <div key={g.id} className="rounded-2xl border border-zinc-900 p-4 space-y-3">
            ...
            <div className="grid grid-cols-2 gap-3 text-xs">
              <div className="space-y-1">
                ...
                >
                  {CATEGORIAS_GASTOS_COMPARTIDOS.map((cat) => (
              ...
              <div className="space-y-1">
                <span className="text-zinc-600 font-mono flex items-center gap-1">
                  Monto
                  {g.recurrente && (
                    <span title="Recurrente: monto copiado, verifica si cambió">
      ...
      <form action={handleAgregar} className="grid grid-cols-2 md:grid-cols-6 gap-4 items-end text-xs">
        ...
        <div className="space-y-1.5">
          ...
          <select
            name="categoria"
            defaultValue={CATEGORIAS_GASTOS_COMPARTIDOS[0]}
            className="w-full bg-zinc-900/50 border border-zinc-800 focus:border-zinc-600 focus:outline-none p-2 rounded-lg text-zinc-300 transition-all"
          >
            {CATEGORIAS_GASTOS_COMPARTIDOS.map((cat) => (
        ...
        <div className="space-y-1.5">
          <label className="text-zinc-500 block font-mono">Monto total</label>
          <input
            type="number"
            name="monto"
            min={0}
        ...
        <label className="flex items-center gap-2 text-zinc-500 font-mono pb-2 col-span-2 md:col-span-1">
          <input type="checkbox" name="recurrente" className="w-3.5 h-3.5 rounded border-zinc-700 bg-zinc-900 accent-amber-500" />
          Recurrente cada mes
        </label>
        <button
          type="submit"
      ...
      {previewUrl && (
        <div className="fixed inset-0 bg-zinc-950/90 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setPreviewUrl(null)}>
          <div className="relative max-w-3xl w-full max-h-[80vh] flex items-center justify-center">
            {previewUrl.toLowerCase().endsWith('.pdf') ? (
              ...
              <img src={previewUrl} alt="Comprobante" className="max-w-full max-h-[75vh] object-contain border border-zinc-800 rounded-lg shadow-2xl" />
            )}
          </div>
        </div>
      )}
